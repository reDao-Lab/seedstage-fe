...
export const MainArea = ({ name, IOUName, veting, idoPrice, ido_network, token_network, total_raise }:IMainArea) => {
  return (
    <div className="ido-box grid grid-cols-7 gap-6">
      <div className="col-span-7 xl:col-span-4 space-y-6">
        ...
        <ValuesInfo idoPrice={idoPrice} ido_network={ido_network} token_network={token_network} total_raise={total_raise} />
        <VetingInfo veting={veting}/>

        <div className="flex w-full justify-between items-center">
          <p className='text-[#d65252]'>This pool requires Rookie tier</p>
          <Button size={"custom"} className='uppercase'>Stake more</Button>
        </div>
      </div>
      <div className="col-span-7 xl:col-span-3 p-3 rounded-[8px] border border-[#3b3b3b] space-y-5">
        <div className="bg-[#e7e7e7] rounded-[8px] p-3 font-medium">
          ...
          <p className='text-center text-[#0a0a0a] text-xl mt-1.5'>30d : 24h : 60m : 60s</p>
        </div>
    ...
    <div className="w-full pl-6">
      ...
      <div className="pt-2 xl:pt-5 flex flex-col xl:flex-row xl:items-center justify-between gap-2.5">
        <div className="border border-[#3b3b3b] rounded-md px-5 py-[8.5px] w-fit text-xs font-medium text-[#bebebe]">
          IOU: <span className='text-balance font-medium text-[#e7e7e7]'>{IOUName}</span>
        </div>
...
const ValuesInfo = ({idoPrice, ido_network, token_network, total_raise }:IValuesInfo) => {
  return <div className="px-3 py-5 rounded-[8px] border border-[#3b3b3b] grid grid-cols-2 gap-3">
...
const VetingInfo = ({ veting } : { veting: string }) => {
  return <div className="p-3 rounded-[8px] border border-[#3b3b3b]">
